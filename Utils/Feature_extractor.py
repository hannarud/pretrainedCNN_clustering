...
class Feature_extractor:
    ...
    def __init__(self, dataset, cnn_architecture, layer):
        ...
        self.dataset_im_path = os.path.join(self.dataset_path, 'images')

        self.dataset_feat_path = os.path.join(self.dataset_path, 'features', str('%s_%s' % (cnn_architecture, layer)))

        self.dataset_im_names = os.listdir(self.dataset_im_path)
        self.n_files = len(self.dataset_im_names)
    ...
    def get_PIL_image(self, image_name):
        return image.load_img(image_name, target_size = self.tgt_size)
    ...
    def extract_and_save_features(self):
        ...
        for im in range(self.n_files):
            ...
            features = self.extract(os.path.join(self.dataset_im_path, self.dataset_im_names[im]))
            features_filename = os.path.splitext(self.dataset_im_names[im])[0]
            feat_file = open(os.path.join(self.dataset_feat_path, str("%s.p" % features_filename)), "wb")
